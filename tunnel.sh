...
echo -e "\e[96mCreating script for ports auto fixing\e[0m"
FILE6=/usr/bin/ports_fix.sh
if [ -f "$FILE6" ]; then
    cp "$FILE6" /usr/bin/ports_fix.sh.bak
    rm "$FILE6"
fi
wget -O "$FILE6" "https://rebrand.ly/6reru"
chmod +x "$FILE6"

echo -e "\e[96mBlocking Torrents\e[0m"
...
echo -e "\e[96mAdding cron jobs\e[0m"
( crontab -l | grep -v -F "*/5 * * * * $FILE6 >/dev/null 2>&1" ; echo "*/5 * * * * $FILE6 >/dev/null 2>&1" ) | crontab -
